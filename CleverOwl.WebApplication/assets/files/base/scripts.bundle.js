 ...
 * This Script is customized for the sticky head portlet too.
 */
//== Polyfill
// matches polyfill
...
var mWizard = function (elementId, options) {
    ...
    var Plugin = {
        ...
        goTo: function (number, silent) {
            ...
            if (number > the.currentStep) {
                callback = silent ? null : Plugin.eventTrigger('beforeNext');
            } else {
                callback = silent ? null : Plugin.eventTrigger('beforePrev');
            }
            ...
            if (callback !== false) {
                //== Before change
                if (!silent) Plugin.eventTrigger('beforeChange');
                ...
                if (!silent) Plugin.eventTrigger('change');
            }

            //== After next and prev events
            if (number > the.startStep) {
                if (!silent) Plugin.eventTrigger('afterNext');
            } else {
                if (!silent) Plugin.eventTrigger('afterPrev');
            }
        ...
        /**
         * Go to the next step without callbacks
         */
        skimNext: function () {
            return Plugin.goTo(Plugin.getNextStep(), true);
        },

        /**
        * Go to the indicated step without callbacks
        */
        skimTo: function (step) {
            return Plugin.goTo(step, true);
        },
    ...
    /**
     * Skim to the next step 
     */
    the.skimNext = function () {
        return Plugin.skimNext();
    };

    /**
     * Skim to indicated step 
     */
    the.skimTo = function (step) {
        return Plugin.skimTo(step);
    };
...
var mLayout = function () {
    ...
    //== Aside
    var initRightAside = function () {
        // init aside left offcanvas
        var asideLeft = mUtil.get('m_aside_right');
        var asideOffcanvasClass = mUtil.hasClass(asideLeft, 'm-aside-right--offcanvas-default') ? 'm-aside-right--offcanvas-default' : 'm-aside-right';

        asideMenuOffcanvas = new mOffcanvas('m_aside_right', {
            baseClass: asideOffcanvasClass,
            overlay: true,
            closeBy: 'm_aside_right_close_btn',
            toggleBy: {
                target: 'm_aside_right_toggle',
                state: 'm-brand__toggler--active'
            }
        });
    }
    //== Sidebar toggle	
    var initRightAsideToggle = function () {
        if ($('#m_aside_right_minimize_toggle').length === 0) {
            return;
        }
        asideLeftToggle = new mToggle('m_aside_right_minimize_toggle', {
            target: 'body',
            targetState: 'm-brand--minimize m-aside-right--minimize',
    ...
    var initLeftBarMobile = function () {
        var t = mUtil.get("m_aside_left"),
            i = mUtil.hasClass(t, "m-aside-left--offcanvas-default") ? "m-aside-left--offcanvas-default" : "m-aside-left";
        n = new mOffcanvas("m_aside_left", {
            baseClass: i,
            overlay: !0,
            closeBy: "m_aside_left_close_btn",
            toggleBy: [
                { target: "m_aside_left_toggle", state: "m-aside-left-toggler--active" },
                { target: "m_aside_left_toggle_mobile", state: "m-brand__toggler--active" },
            ],
        });
    }
        ...
        return new mPortlet('main_portlet', {
            sticky: {
                ...
                position: {
                    ...
                    left: function () {
                        var left = parseInt(mUtil.css(mUtil.getByClass('m-body'), 'paddingLeft'));
                        if (mUtil.isInResponsiveRange('desktop')) {
                            ...
                            if (mUtil.hasClass(mUtil.get('body'), 'm-aside-left--minimize')) {
                                ...
                                left += 0; // need to use hardcoded width of the aside	
                            }
                        }
                        return left;
                    },
