    ...
    var initButtons = function () {
        ...
        btnNext.on('click', function (e) {
            ...
            if (wizard.getStep() === 2) {
                ...
                if (!phoneNumberValid()) {
                    swal(CrowdContentParticipateAlertPhoneNumberNotValidTitleSwal, CrowdContentParticipateAlertPhoneNumberNotValidTextSwal, "info").then((result) => {
                        $([document.documentElement, document.body]).animate({
                            scrollTop: $("#mobilePhoneHeading").offset().top - 80
                        });
                    });
                    mApp.unblock(wizardEl);
                    mApp.unprogress(btnNext);
                ...
                if (!informationAccuracyAcknowledged()) {
                    swal(CrowdContentParticipateAlertAcceptTermsTitleSwal, CrowdContentParticipateAlertAcceptTermsTextSwal, "info").then((result) => {
                        $([document.documentElement, document.body]).animate({
                            scrollTop: $("#acknowledgeInformationAccuracyContainer").offset().top - 80
                        });
                    });
                    mApp.unblock(wizardEl);
                    mApp.unprogress(btnNext);
            ...
            formEl.ajaxSubmit({
                ...
                error: function (xhr, status, error) {
                    ...
                    swal(CrowdContentParticipateAlertErrorTitleSwal, CrowdContentParticipateAlertErrorBodySwal, "error");
