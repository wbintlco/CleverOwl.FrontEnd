...
var CrowdContentParticipateWizard = function () {
    ...
    var initButtons = function () {
        ...
        btnNext.on('click', function (e) {
            ...
            console.log("btnNextbtnNextbtnNext");
            if (wizard.getStep() === 2) {
                GradeDropDown = $("#GradeDropDown option:selected").val();
                SubjectDropDown = $("#SubjectDropDown option:selected").val();
                
               
                $(".field-validation-valid").attr('style', 'display: none !important');
                if (GradeDropDown == '') {
                    $(".GradeDropDown").attr('style', 'display: inline-block !important');
                }

                if (SubjectDropDown == '') {
                    $(".SubjectDropDown").attr('style', 'display: inline-block !important');
                }

               

                if (GradeDropDown == '' || SubjectDropDown == '') {
                    mUtil.scrollTop();
                    mApp.unblock(wizardEl);
                    mApp.unprogress(btnNext);
                    return;
